...
let [
	actionBaseinfoGridList, actionBaseinfoGridAdd, actionBaseinfoGridDelete ]
	= _actionCreator.init(['gridList', 'gridAdd', 'gridDelete'])
...
export default {
	...
	methods: {
      ...
      this.loadCode = 10
			actionBaseinfoGridList(params, this.source.token)
				.then((res) => {
      ...
      if (this.selectionList.length <= 0) {
        ...
        this.$confirm('请确认删除数据！', '提示', {
          ...
          type: 'warning'
					...
					this.$store.state.loading = true
      		this.loadCode = 10
					actionBaseinfoGridDelete(params, this.source.token)
						.then(res => {
							if (res.code == "0") {
